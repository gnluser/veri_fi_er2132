...
class Information_Frame():
    ...
    def create_subequipment_list_box(self, canvas, x, y, node_instance, new_equipment):  # ,node_label):
        ...
        try:
            self.subequipment_select_button=Button(self.window_frame,text="->",command= self.load_subeqpmnt_property_window_box( canvas,node_instance, new_equipment))
        except:
            print("subeqpmnt select button")
        self.subequipment_select_button.pack(side="bottom")
        self.subequipment_list_box.bind('<<ListboxSelect>>',lambda event: self.display_subparts_of_respective_subequipments(event, canvas, node_instance, new_equipment,x,y))
    ...
    def create_subpart_window(self, canvas, x, y, node_instance):
        ...
        self.subpart_list_box=Listbox(subpart_frame, selectmode=MULTIPLE, bd=5, exportselection=0, width=10,
                                             height=15, bg="deepskyblue")
        self.subpart_select_button=Button(subpart_frame,text="->",command=lambda : self.selecting_multiple_subparts(event,self.subpart_list_box,canvas,node_instance))
    ...
    def selecting_multiple_subparts(self,event,widget,canvas,node_instance):
    ...
    def equipment_property_load(self, event, canvas, x, y, node_instance):
        ...
        self.subequipment_window_load(new_equipment, canvas, x, y, node_instance,equipment_name)

    def subequipment_window_load(self, new_equipment, canvas, x, y, node_instance,equipment_name):
        try:
            self.subequipment_list_box.delete(0, END)
        except:
            print("not new subequipment box")
        self.create_subequipment_list_box(canvas, x, y, node_instance, new_equipment)
        ...
        if new_equipment.equipment_properties[subequipment_name] == "":
            ...
            self.subequipment_list_box.insert(END, new_equipment.equipment_properties[subequipment_name])
        '''


    def display_subparts_of_respective_subequipments(self,event,canvas,node_instance,new_equipment,x,y):
        ...
        if new_subeqpmnt_option != "":
            for x in new_subeqpmnt_option:
                ...
                self.call_subpart_create_function(new_equipment, canvas, node_instance, x, y)
    ...
    def call_subpart_create_function(self,new_subequipment,canvas,node_instance,x,y):
        ...
        subpart_list = self.current_vendor_instance.all_eqpmnt_subeqpmnt_and_parts_dictionary[new_subequipment][subeqpmnt_subpart]
        print(self.current_vendor_instance.all_eqpmnt_subeqpmnt_and_parts_dictionary)
        print(self.current_vendor_instance.all_eqpmnt_subeqpmnt_and_parts_dictionary[new_subequipment])
        if subpart_list != Default:
            self.create_subpart_window( canvas, x, y, node_instance)

        else:
            print("add code to select the ports and properties of subequipment")

    def load_subeqpmnt_property_window_box(self, canvas, node_instance, new_equipment):
        ...
        for index in indices:
            new_subequipment = self.subequipment_list_box.get(int(index))
